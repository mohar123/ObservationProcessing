...
times = range (0, 24, args.interval) # list of time offsets:  eg 0Z, 6Z, 12Z, 18Z
...
for date in dates:
  ...
  for tcenter in times:
    ...
    #print "daystamp = ", daystamp
    #print "from write_ro_hdr:"
    os.system ('./write_ro_hdr '+daystamp)
    print ('end write_ro_hdr')
    ...
    for mission in mission_list:
      ...
      for f in files:
        ...
        #(fyr, fdoy, fhr, fmin,fsec) = [int(i) for i in re.search(r'(\d\d\d\d)\.(\d\d\d)\.(\d\d)\.(\d\d).(\d\d)', f).groups()]
        #tfile = TimeClass().set_yrdoyhms_gps(fyr, fdoy, fhr, fmin, fsec).get_gps() # Time of occultation in GPS seconds
